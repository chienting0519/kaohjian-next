// 1. 引入型別 (Import)
import { OperatingHours, ServiceItem, Symptom, Doctor, Clinic, AllianceHospital } from './types';

// ★★★ 關鍵修正：將引入的型別再次匯出，讓其他檔案也能從這裡讀取到 ★★★
export type { OperatingHours, ServiceItem, Symptom, Doctor, Clinic, AllianceHospital };

// 2. 診所基本資訊
export const CLINIC_INFO = {
  name: "高健診所",
  slogan: "高雄市民的健康就交給高健診所",
  address: "812高雄市小港區沿海一路88號",
  phone: "07 802 7828",
  bookingLink: "https://lin.ee/RIY5AtG",
  mapLink: "https://www.google.com/maps/search/?api=1&query=高健診所+高雄市小港區沿海一路88號"
};

// 3. 營業時間
export const OPERATING_HOURS: OperatingHours[] = [
  { day: "星期一", time: "08:00–22:00", isOpen: true },
  { day: "星期二", time: "08:00–16:00", isOpen: true },
  { day: "星期三", time: "08:00–22:00", isOpen: true },
  { day: "星期四", time: "08:00–16:00", isOpen: true },
  { day: "星期五", time: "08:00–22:00", isOpen: true },
  { day: "星期六", time: "08:00–16:00", isOpen: true },
  { day: "星期日", time: "休息", isOpen: false },
];

// 4. 門診時間說明
export const SCHEDULE_CONTEXT = `
詳細門診與洗腎時間:

[洗腎透析時段]
早班: 週一至週六 07:00 - 11:30
午班: 週一至週六 11:30 - 16:30
晚班: 週一、三、五 16:30 - 22:00

[門診時段]
早診: 週一至週六 09:30 - 12:00
午診: 
  - 週一、三、五: 14:00 - 17:00
  - 週二、週六: 12:00 - 16:00 (中午不休息)
  - 週四: 14:00 - 16:30
晚診: 週一、三、五 18:00 - 21:00
`;

// 5. 服務項目
export const SERVICES: ServiceItem[] = [
  {
    title: "血液透析中心",
    description: "醫學中心等級的洗腎環境，採用德國原裝透析機與雙重RO純水處理，提供小港、鳳山、林園地區腎友最安心的選擇。",
    icon: "Activity",
    items: [
      "高效率血液透析 (HD/HDF) - 改善疲倦與皮膚搔癢",
      "雙重逆滲透 (RO) 純水系統 - 嚴格控制細菌與內毒素",
      "配備每日消毒殺菌個人床單棉被 - 舒適透析空間",
      "溫馨接送車隊 (小港/鳳山/林園/大寮/前鎮)",
      "資深醫療護理團隊照護"
    ]
  },
  {
    title: "慢性病整合照護",
    description: "糖尿病與高血壓是腎臟病的主因。我們提供一站式的三高管理，不僅開藥，更教您如何透過飲食逆轉數值。",
    icon: "Stethoscope",
    items: [
      "糖尿病共照網認證 - 施打胰島素與血糖監測教學",
      "高血壓藥物精準調控 - 保護腎臟血管",
      "高血脂與膽固醇管理 - 預防心血管併發症",
      "痛風與高尿酸治療 - 減少腎結石風險",
      "一般內科 (感冒、腸胃炎、過敏、氣喘)"
    ]
  },
  {
    title: "腎臟健康檢查",
    description: "腎臟病早期沒有症狀。我們引進精密檢測儀器，只需微量尿液與血液，即可早期發現蛋白尿與腎功能異常。",
    icon: "ClipboardList",
    items: [
      "免費成人健康檢查 (30歲以上每3年1次)",
      "精準微量白蛋白尿檢測 (ACR) - 糖尿病腎病變篩檢",
      "腎絲球過濾率 (eGFR) 追蹤",
      "老人健檢與四癌篩檢 (大腸癌糞便潛血)",
      "腎臟超音波與心電圖檢查"
    ]
  },
  {
    title: "預防醫學與營養",
    description: "除了藥物治療，我們更重視「預防」。透過營養點滴與生活型態醫學，提升您的免疫力與修復力。",
    icon: "ShieldCheck",
    items: [
      "低蛋白飲食衛教 - 延緩洗腎時程",
      "ILIB 靜脈雷射 - 促進血液循環",
      "自費減重門診 (猛健樂) - 減輕腎臟負擔",
      "個人化營養點滴與維生素補充",
      "B、C 型肝炎追蹤與篩檢"
    ]
  }
];

// 6. 腎臟病症狀檢測
export const KIDNEY_SYMPTOMS: Symptom[] = [
  { id: 'foam', question: '小便時是否有不易消散的泡沫 (蛋白尿)?', riskWeight: 2 },
  { id: 'edema', question: '最近是否感覺下肢或臉部容易水腫?', riskWeight: 2 },
  { id: 'fatigue', question: '是否經常感到不明原因的疲倦或貧血?', riskWeight: 1 },
  { id: 'bp', question: '是否有高血壓且難以控制?', riskWeight: 2 },
  { id: 'urine_color', question: '尿液顏色是否異常 (如深茶色、血色)?', riskWeight: 3 },
  { id: 'pain', question: '是否有腰部兩側痠痛的感覺?', riskWeight: 1 },
  { id: 'family', question: '直系親屬中是否有腎臟病史?', riskWeight: 2 },
  { id: 'diabetes', question: '本身是否有糖尿病病史?', riskWeight: 3 },
];

// 定義文章介面 (這是你在本檔案內定義的，所以 export 沒問題)
export interface Article {
  id: string;
  slug: string;
  title: string;
  summary: string;
  date: string;
  category: string;
  tags: string[];
  content: string;
  imageUrl?: string;
}

// 7. 衛教文章資料
export const ARTICLES: Article[] = [
  {
    id: '7',
    slug: 'hemodialysis-process-and-insurance-2025',
    title: '【洗腎懶人包】血液透析不可怕！醫師圖解流程與健保給付解析 (2025最新版)',
    summary: '高健診所為您解析血液透析（洗腎）流程、飲食禁忌與健保給付。破解洗腎迷思，提供高雄地區最專業的透析照護資訊。',
    date: '2025.12.25',
    category: '透析治療',
    tags: ['血液透析', '健保給付', '洗腎流程', '高雄腎臟科'],
    imageUrl: "https://images.unsplash.com/photo-1579684453423-f84349ca60df?auto=format&fit=crop&w=800&q=80",
    content: `當醫師告知您的腎絲球過濾率 (eGFR) 已低於 15，必須開始準備透析治療時，恐懼是正常的反應。「洗腎是不是一輩子就毀了？」、「會不會很痛？」、「還能正常工作嗎？」這些問題困擾著無數病友。

事實上，現代血液透析技術已大幅進步。在**高健診所**，我們見證了無數腎友在接受正規治療後，重拾生活品質，甚至比患病初期更有精神。本篇將用數據與實證，為您拆解血液透析的真相。

### 為什麼我需要洗腎？不洗會怎樣？
大多數患者對於進入透析階段（俗稱洗腎）有強烈的抗拒感。核心痛點在於對「尿毒症」併發症的未知。

* **核心問題：** 許多患者認為只要沒有明顯不舒服，就可以拖延洗腎時間。
* **醫學現實：** 當腎臟功能僅剩不到 10% 時，體內毒素與水分無法排出，可能引發：
    - **高血鉀症：** 可能導致突發性心律不整甚至猝死。
    - **肺水腫：** 導致呼吸衰竭，這是許多末期腎病患者掛急診的主因。
    - **尿毒性腦病變：** 導致意識不清、昏迷。

**⚠️ 數據警示：** 根據 *台灣腎臟醫學會 (TSN)* 的年報數據顯示，延遲透析的患者，其緊急住院率比按時透析者高出 **2.5 倍**，且初期死亡率顯著增加。提早建立血管通路（廔管），能大幅降低感染風險。

### 現代血液透析 (Hemodialysis) 的優勢
面對末期腎臟病，選擇合適的透析方式是重獲新生的關鍵。高健診所專注於提供高品質的「血液透析」服務，我們透過以下三點來解決患者的擔憂：

**1. 高效率的毒素清除 (High-efficiency HD)**
相比於傳統透析，現代**高通量人工腎臟 (High-Flux Dialyzer)** 能更有效地清除中大分子毒素（如 β2-微球蛋白），這直接減少了透析後的皮膚搔癢與骨頭痠痛問題。

**2. 精準的脫水控制 (Fluid Management)**
透過 BCM (身體組成監測儀) 等輔助設備，我們可以精確計算患者的「乾體重」，避免洗太乾導致抽筋、掉血壓，或洗不乾淨導致水腫。

**3. 醫療團隊的全程照護**
不同於腹膜透析需要患者在家自行操作，血液透析由專業護理師全程操作。在高健診所，醫師會在透析過程中巡視，即時處理任何不適，這對於年長或有共病（如糖尿病、高血壓）的患者來說，安全性最高。

### 數據會說話：臨床指標與實證
為了證實上述解決方案的有效性，我們引用具體的臨床指標與權威數據：

**存活率數據**
根據 **美國腎臟資料系統 (USRDS)** 及台灣健保資料庫統計，台灣透析患者的五年存活率在國際上名列前茅。接受高品質血液透析的患者，其生活品質量表 (SF-36) 分數顯著高於未治療的尿毒症患者。

**透析清除率指標 (Kt/V)**
這是評估透析是否乾淨的黃金標準。
標準：臨床指引建議 Kt/V 至少應大於 **1.2**。在高健診所，我們透過嚴格的水質控制與個別化流速設定，致力於讓每位腎友的 Kt/V 達到最佳標準 (通常目標 > 1.4)，確保毒素清除徹底。

**營養指標 (Albumin)**
血清白蛋白是預後的關鍵指標。透過我們的營養衛教介入，協助患者攝取足夠的優質蛋白質（每公斤體重 1.2 克），維持體力與免疫力。

### 患者最想知道的事 (Q&A)
以下整理了診間最常被問到的問題：

**Q1：洗腎後，飲食要完全無鹽嗎？**
A：不需要完全無鹽，但要「低鈉」。極端無鹽會導致食慾不振。重點是避免「加工食品」（如罐頭、香腸），改用天然辛香料（蔥、薑、蒜）提味。

**Q2：開始洗腎後，還能出國旅遊嗎？**
A：絕對可以！這稱為「透析旅遊」。只要病情穩定，您可以提前聯繫目的地的透析中心安排臨時透析。台灣的透析品質世界聞名，許多國外腎友也會來台旅遊並透析。

**Q3：高健診所的血液透析有健保給付嗎？**
A：是的。台灣全民健保對於重大傷病（末期腎臟病）有完整給付。患者領有重大傷病卡後，原本昂貴的透析費用將由健保負擔，大幅減輕經濟壓力，讓您只需專注於治療與保養。

### 我們與您並肩作戰
洗腎不是人生的終點，而是「腎臟功能」的接力賽。透過高健診所專業團隊的協助，您依然可以擁有色彩斑斕的人生。
如果您或家人有相關疑慮，歡迎直接來院諮詢，讓我們一起制定最適合的治療計畫。

(資料來源參考：台灣腎臟醫學會 2024 年報、美國國家腎臟基金會 NKF)`
  },
  {
    id: '6',
    slug: 'bubble-urine-kidney-warning-signs',
    title: '泡泡尿是洗腎前兆？高雄腎臟醫師揭密：3 個不可忽視的警訊',
    summary: '早上起床小便有泡泡，是腎臟出問題了嗎？高健診所醫療團隊為您解析「蛋白尿」的真實樣貌。學會分辨「啤酒泡沫」，掌握檢測黃金期，別讓沈默殺手找上門。',
    date: '2025.12.08',
    category: '症狀自我檢測',
    tags: ['泡泡尿', '蛋白尿', '洗腎前兆', '高雄腎臟科'],
    imageUrl: "https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?auto=format&fit=crop&w=800&q=80",
    content: `在我們位於高雄小港的高健診所門診中，最常聽到患者焦急地問：「醫師，我今天早上小便有泡泡，是不是要洗腎了？」腎臟病素有「沉默殺手」之稱，根據台灣腎臟醫學會的統計，台灣慢性腎臟病 (CKD) 的盛行率高達 11.9%，但因初期症狀不明顯，許多人往往錯失了黃金治療期。

這篇文章將由高健診所的醫療團隊，為您解析如何判斷真正的「蛋白尿」，以及日常生活中該如何透過飲食與檢測，守護您的腎臟健康。

### 核心問題：泡泡尿真的等於腎虧嗎？
首先，我們要釐清一個觀念：**有泡泡不一定就是腎臟病，但腎臟病往往伴隨泡泡尿。**
尿液產生泡沫的原因很多，包括尿流速度快、飲水過少導致尿液濃縮，甚至是劇烈運動後攝取過多蛋白質。

**如何分辨危險的「蛋白尿」？**
我們建議患者在家進行簡單的「10 分鐘觀察法」：
1.  排尿後，觀察馬桶內的泡沫。
2.  若泡沫大且容易消散，通常無大礙。
3.  **若泡沫細緻如啤酒泡沫，且停留超過 10 分鐘仍未消散**，這極有可能是尿蛋白過高的警訊，代表腎臟的過濾功能（腎絲球）可能受損，導致蛋白質流失到尿液中。

### 解決方案：從檢測到控制的黃金三部曲

如果您發現上述徵兆，或本身是糖尿病、高血壓的高風險族群，請務必遵循以下步驟：

**第一步：精準檢測（ACR 與 eGFR）**
一般民眾僅看「肌酸酐 (Creatinine)」指數其實不夠。在高健診所，我們建議進行更精密的 **UACR (尿液微量白蛋白/肌酸酐比值)** 檢測。根據國健署指引，UACR 能比傳統尿蛋白試紙更早發現腎臟病變，尤其對於糖尿病患者，這是預防進入洗腎階段的關鍵指標。

**第二步：控制三高，阻斷惡化源頭**
高血壓與高血糖是破壞腎臟血管的兩大元兇。研究顯示，嚴格控制血壓在 130/80 mmHg 以下，能顯著延緩腎功能下降的速度。我們診所提供整合性的糖尿病與高血壓照護門診，不只是開藥，更包含營養師的飲食衛教。

**第三步：低蛋白飲食原則**
對於慢性腎臟病第三期以後的患者，蛋白質攝取量需精算。這並不代表不能吃肉，而是要選擇「高生物價蛋白質」（如雞蛋、魚肉、黃豆製品），減少產生過多含氮廢物，減輕腎臟負擔。

### 臨床證據與在地觀點
根據台灣健保署的公開數據，進入透析（洗腎）治療的患者中，有超過 45% 是由糖尿病腎病變轉化而來。這意味著，如果在糖尿病初期就能妥善控制血糖並定期監測腎功能，將近一半的洗腎案例是可以被延緩甚至避免的。

在高健診所，我們引進了醫學中心等級的透析技術與檢測設備，目的就是為了在「透析前」階段攔截病程，或在進入「透析後」階段提供高品質的血液透析服務。

**結語**
腎臟健康不可逆，但可以被管理。高健診所作為高雄在地深耕的腎臟專科，致力於提供最溫暖且專業的醫療依靠。如果您對自己的腎臟健康有疑慮，別再猶豫，立即預約諮詢。`
  },
  {
    id: '5',
    slug: 'kidney-failure-warning-signs',
    title: '【名醫專欄】喘到只能趴著睡？出現這 5 大徵兆，恐是腎臟在喊救命！',
    summary: '台灣素有「洗腎王國」之稱。腎臟不只是排尿的器官，更掌管全身水分代謝。若出現嚴重水腫、泡泡尿、不明疲倦等徵兆，恐是腎臟在喊救命。',
    date: '2024.12.06',
    category: '慢性病防治',
    tags: ['腎臟病徵兆', '水腫', '洗腎前兆', '名醫專欄'],
    imageUrl: "https://images.unsplash.com/photo-1505751172876-fa1923c5c528?auto=format&fit=crop&w=800&q=80",
    content: `台灣素有「洗腎王國」之稱，許多民眾常有疑問：「我吃了一輩子都沒事，為什麼突然要洗腎？」

國內腎臟移植權威李伯璋醫師在訪談中指出，腎臟不只是排尿的器官，更掌管著全身的水分代謝。臨床上曾有案例，患者因腎衰竭導致嚴重積水，毒素排不出去，晚上完全無法平躺睡覺，只能趴在桌上睡（端坐呼吸），檢查後發現心臟因負荷過重，已經腫得像一顆「小玉西瓜」。

這告訴我們：**腎臟如果不健康，心臟與全身都會受害。**

⚠️ **自我檢測：這 5 個「求救訊號」你中了嗎？**
腎臟是沉默的器官，等到不舒服往往已是末期。高健診所提醒您，如果您居住在高雄小港、林園、鳳山地區，且出現以下徵兆，請務必提高警覺：

1. **嚴重水腫、走路會喘**
按壓小腿後皮膚回彈慢，或是走幾步路就氣喘吁吁，可能是肺部積水。

2. **泡泡尿（經久不散）**
小便出現細緻且不易消散的泡沫，代表腎絲球受損，蛋白質流失（蛋白尿）。

3. **莫名疲倦、貧血**
整天昏昏欲睡、臉色蒼白？因為腎臟負責造血功能，受損會導致嚴重貧血。

4. **高血壓降不下來**
吃藥也難控制的高血壓，往往是腎臟病變的兇手，也是結果。

5. **皮膚搔癢、半夜抽筋**
體內磷離子無法排出，會導致皮膚深層搔癢與電解質失衡。

🚫 **兩大洗腎元兇：不是吃太鹹，而是...**
根據統計，台灣人洗腎的兩大主因其實是：
* **第一名：糖尿病控制不佳**（高血糖破壞腎臟微血管）。
* **第二名：濫用藥物**（長期服用不明中草藥或強力止痛藥）。

👨‍⚕️ **高健診所院長叮嚀：還有機會「逆轉」嗎？**
被診斷出腎功能異常，不代表人生就此黑白！ 如果是早期的慢性腎臟病，或是藥物引起的急性損傷，只要及早發現、停止傷腎行為、配合專業藥物治療，腎功能是有機會穩定甚至恢復的。

🏥 **小港在地的腎臟守護者｜高健診所**
不想走到洗腎這一步？「定期檢查」是唯一的預防途徑。

我們深耕小港、林園生活圈，提供：
* **免費成人健檢**：30歲以上每3年一次，精準檢測腎功能與蛋白尿。
* **糖尿病共照網**：專業團隊協助控糖，預防腎病變。
* **五星級血液透析中心**：若需透析治療，我們提供德國原裝設備與溫馨接送車隊（服務範圍：小港、林園、大寮、鳳山、前鎮）。

別讓沈默的殺手找上門，現在就花一分鐘檢視您的健康！`
  },
  {
    id: '1',
    slug: 'foamy-urine-kidney-disease-check',
    title: '泡泡尿是腎臟病嗎？3分鐘教你自我判斷',
    summary: '許多人看到小便有泡泡就擔心是腎虧或腎臟病。其實泡泡尿不等於蛋白尿，透過簡單的觀察原則，您可以初步篩檢風險。',
    date: '2024.03.15',
    category: '症狀與迷思',
    tags: ['蛋白尿', '腎臟病', '自我檢測'],
    imageUrl: "https://images.unsplash.com/photo-1530497610245-94d3c16cda28?auto=format&fit=crop&w=800&q=80",
    content: `看到馬桶裡有泡泡，先別慌張！「泡泡尿」確實是腎臟病的警訊之一，但並非所有泡泡都是蛋白尿。

以下教您 3 個觀察重點，自我判斷風險：

1. **泡泡的持久度**
如果泡泡在 10-15 秒內就自然消失，通常只是因為排尿衝擊力道較大，或是水喝太少導致尿液濃縮，這屬於正常現象。但如果泡泡像「啤酒泡沫」一樣綿密，且過了幾分鐘還不消散，就要提高警覺了。

2. **是否有合併其他症狀**
如果除了泡泡尿，您還伴隨有「下肢水腫」、「容易疲倦」、「高血壓」或「臉部浮腫」等症狀，這更有可能是腎臟功能受損的跡象。

3. **定期檢查才是王道**
最準確的方法是到診所做一次尿液檢查。透過試紙檢測，我們可以準確知道尿液中的蛋白質含量是否超標。

**醫師建議：**
如果您發現泡泡不易消散持續超過兩三天，建議儘早至高健診所掛號，簡單的尿液檢查就能讓您安心，早期發現早期治療效果最好！`
  },
  {
    id: '2',
    slug: 'diabetes-kidney-protection-diet',
    title: '糖尿病友如何護腎？飲食控制是關鍵',
    summary: '糖尿病是造成台灣洗腎人口第一名的主因。控制血糖與血壓，搭配低蛋白飲食原則，能有效延緩腎功能惡化。',
    date: '2024.02.28',
    category: '慢性病管理',
    tags: ['糖尿病', '飲食衛教', '三高控制'],
    imageUrl: "https://images.unsplash.com/photo-1511688878353-3a2f5be94cd7?auto=format&fit=crop&w=800&q=80",
    content: `台灣有接近一半的洗腎患者，起因都是糖尿病控制不佳。高血糖會導致血管病變，進而破壞腎臟的過濾功能。

糖尿病友護腎的三大關鍵：

1. **嚴格控制血糖與血壓**
糖化血色素(HbA1c)建議控制在 7.0% 以下，血壓則建議控制在 130/80 mmHg 以下。這是保護血管與腎臟最基本也最重要的防線。

2. **定期追蹤腎功能**
建議每 3-6 個月檢查一次「尿液微量白蛋白」與「腎絲球過濾率 (eGFR)」。不要等到出現症狀才檢查，因為腎臟病早期通常是沒有感覺的。

3. **正確的飲食觀念**
* **低鹽**：減少調味料，預防高血壓。
* **優質蛋白**：若腎功能已開始下降，需限制蛋白質攝取量，並以魚、蛋、肉、豆類等高生理價蛋白質為主。
* **避免加工食品**：香腸、火腿、罐頭等含磷量極高，會加速腎臟衰竭。

在高健診所，我們會為每位糖友制定個人化的追蹤計畫，結合藥物與飲食衛教，陪您一起守護腎臟健康。`
  },
  {
    id: '3',
    slug: 'hemodialysis-peritoneal-dialysis-intro',
    title: '洗腎不可怕！認識血液透析與腹膜透析',
    summary: '當腎臟功能無法維持身體運作時，透析治療能幫助您延續生活品質。高健診所提供高品質的血液透析環境，讓病患安心治療。',
    date: '2024.01.10',
    category: '透析治療',
    tags: ['血液透析', '洗腎', '衛教'],
    imageUrl: "https://images.unsplash.com/photo-1579684385127-1ef15d508118?auto=format&fit=crop&w=800&q=80",
    content: `當腎臟功能剩下不到正常的 10-15%，且出現尿毒症狀（如噁心、嘔吐、呼吸困難、嚴重水腫）時，醫師會建議開始準備透析治療，也就是俗稱的「洗腎」。

許多人聽到洗腎就覺得人生黑白，其實現在的透析技術非常進步，只要配合治療，腎友依然可以出國旅遊、享受美食、擁有良好的生活品質。

**常見的兩種透析方式：**

1. **血液透析 (Hemodialysis, HD)**
* **方式**：每週三次到診所，由機器將血液引出，過濾毒素與水分後再送回體內。
* **優點**：由專業醫護人員操作，患者較輕鬆；家裡不需要堆放藥水。
* **高健診所的優勢**：我們擁有獨棟透析大樓，使用高階透析用水處理系統與先進透析機，並有資深腎臟專科醫師全程照護。

2. **腹膜透析 (Peritoneal Dialysis, PD)**
* **方式**：在家中自行更換藥水，利用腹膜過濾毒素。
* **優點**：時間彈性，不用常跑醫院。
* **缺點**：需要家中空間存放藥水，且需注意感染風險。

**3. 貼心的交通接送，治療無距離**
我們了解許多腎友行動不便或家人忙碌。高健診所特別提供**溫馨接送服務**，服務範圍涵蓋**小港、鳳山、林園、大寮**等地區。只需一通電話，我們就用家人般的溫暖，載您平安往返透析中心。

面對洗腎，最重要的是心態調整。高健診所的醫療團隊會像家人一樣陪伴您，讓透析治療成為您延續生命活力的助力，而不是阻力。`
  },
  {
    id: '4',
    slug: 'gout-high-uric-acid-kidney-damage',
    title: '痛風與高尿酸：隱形的腎臟殺手',
    summary: '高尿酸不僅會引起關節疼痛，長期結晶沉積更會傷害腎臟間質。定期追蹤尿酸值並多喝水，是預防關鍵。',
    date: '2023.12.05',
    category: '疾病預防',
    tags: ['痛風', '高尿酸', '預防醫學'],
    imageUrl: "https://images.unsplash.com/photo-1584362917165-526a968579e8?auto=format&fit=crop&w=800&q=80",
    content: `大家都知道痛風發作時關節會痛得受不了，但您知道嗎？高尿酸血症其實也是腎臟的沈默殺手！

**尿酸如何傷害腎臟？**
當血液中尿酸過高，尿酸結晶不僅會沈積在關節，也會沈積在腎臟的間質組織中，造成「痛風性腎病變」，長期下來會導致腎功能衰退，甚至洗腎。此外，高尿酸也容易形成尿路結石，阻塞尿路進而傷腎。

**預防高尿酸的三不一要：**

1. **不喝含糖飲料**：果糖會加速尿酸合成，手搖飲是大忌。
2. **不飲酒**：酒精會影響尿酸代謝，尤其是啤酒與烈酒。
3. **不吃高普林食物**：如內臟、濃縮肉汁、大量海鮮等。
4. **要多喝水**：一般人每天建議喝水 2000cc 以上，幫助尿酸排出。（注意：若已被醫師告知需限水的腎友，請遵照醫囑）

若您健檢發現尿酸偏高（男性 > 7.0 mg/dL，女性 > 6.0 mg/dL），即使還沒痛風發作，也建議至門診諮詢。我們會評估是否需要藥物控制，保護您的腎臟免受結晶傷害。`
  }
];


// 8. 醫療團隊
export const MEDICAL_TEAM: Doctor[] = [
  {
    name: "洪錦傳",
    title: "院長 / 主治醫師",
    specialties: ["腎臟專科", "內科", "糖尿病共同照護"],
    experience: [
      "高雄榮總內科部 腎臟科主治醫師",
      "屏東東港安泰醫院 腎臟科主治醫師"
    ],
    certifications: [
      "台灣內科醫學會 專科醫師",
      "台灣腎臟醫學會 專科醫師",
      "中華民國醫用超音波學會 會員"
    ]
  },
  {
    name: "吳美美",
    title: "主治醫師",
    specialties: ["腎臟專科", "內科", "三高慢性腎病防治"],
    experience: [
      "台中榮民總院 腎臟內科醫師",
      "高雄聖功醫院 腎臟内科主治醫師",
      "高雄三泰醫院 腎臟內科醫師"
    ],
    certifications: [
      "台灣內科醫學會 專科醫師",
      "台灣腎臟醫學會 專科醫師",
      "糖尿病共同照護網 認證醫師"
    ]
  },
  {
    name: "護理照護團隊",
    title: "20 年資歷",
    specialties: ["急重症照護", "雙證照護理", "門診照護"],
    experience: [
      "擁有 20 年以上 臨床血液透析照護經驗",
      "醫學中心等級照護標準，專注併發症預防",
      "具備血液透析與腹膜透析雙專業護理證照",
      "提供重症諮詢與在地門診服務"
    ],
    certifications: [
      "國家級護理師執照",
      "血液透析與腹膜透析專科護理認證",
      "ACLS 高級心臟救命術認證"
    ]
  }
];

// 9. 高雄診所名冊
export const KAOHSIUNG_CLINICS_LIST: Clinic[] = [
  { name: "高健診所", address: "高雄市小港區沿海一路88號", phone: "07-8027828" },
  { name: "惠仁醫院", address: "高雄市新興區中山一路67-2號", phone: "07-2010196" },
  { name: "李一鳴內科診所", address: "高雄市新興區復興一路44號", phone: "07-2266893" },
  { name: "高欣診所", address: "高雄市新興區復興一路44號2樓", phone: "07-2266893" },
  { name: "好生診所", address: "高雄市前金區七賢二路183號", phone: "07-2867000" },
  { name: "上順診所", address: "高雄市前金區中一路225號3樓", phone: "07-2725227" },
  { name: "高雄市立聯合醫院大同院區", address: "高雄市前金區中華三路68號", phone: "07-2618131#2321" },
  { name: "五福診所", address: "高雄市前金區中華四路318號1樓", phone: "07-2213933" },
  { name: "阮綜合醫療社團法人阮綜合醫院", address: "高雄市苓雅區成功一路62號", phone: "07-3342765" },
  { name: "國軍高雄總醫院", address: "高雄市苓雅區中正一路2號", phone: "07-7496592" },
  { name: "財團法人天主教聖功醫院", address: "高雄市苓雅區建國一路352號", phone: "07-2249423" },
  { name: "高雄市立民生醫院", address: "高雄市苓雅區凱旋二路134號", phone: "07-7511131" },
  { name: "財團法人高雄基督教信義醫院", address: "高雄市苓雅區華新街86號", phone: "07-3321111#238" },
  { name: "聖安診所", address: "高雄市苓雅區大順三路186號", phone: "07-7277057" },
  { name: "愛欣診所", address: "高雄市苓雅區正義路136號5樓", phone: "07-7497077" },
  { name: "家綾診所", address: "高雄市苓雅區海邊路66號", phone: "07-2412492" },
  { name: "吳三江內科診所", address: "高雄市苓雅區憲政路240號", phone: "07-2222088" },
  { name: "昱泰診所", address: "高雄市苓雅區興中一路227號", phone: "07-5378768" },
  { name: "佳明診所", address: "高雄市苓雅區中正一路136號", phone: "07-7263727" },
  { name: "保順診所", address: "高雄市苓雅區成功一路145號", phone: "07-2695856" },
  { name: "新鴻遠診所", address: "高雄市苓雅區建國一路113-2號1樓", phone: "07-7229860" },
  { name: "高雄市立聯合醫院美術館院區", address: "高雄市鼓山區中華一路976號", phone: "07-5552565" },
  { name: "腎美診所", address: "高雄市鼓山區中華一路336號", phone: "07-5544900" },
  { name: "高雄市立旗津醫院", address: "高雄市旗津區廟前路1-1號", phone: "07-5715101" },
  { name: "博佑診所", address: "高雄市前鎮區一心一路243號", phone: "07-3334591" },
  { name: "吉泰內科診所", address: "高雄市前鎮區保泰路419號2F", phone: "07-7616000#200" },
  { name: "安順診所", address: "高雄市前鎮區后安路263號1樓", phone: "07-8128320" },
  { name: "佑鎮診所", address: "高雄市前鎮區樹人路128號", phone: "07-8121606" },
  { name: "高雄醫學大學附設醫院", address: "高雄市三民區自由一路100號", phone: "07-3121101" },
  { name: "南山醫院", address: "高雄市三民區建國三路151號", phone: "07-2882105" },
  { name: "祐生醫院", address: "高雄市三民區建國三路60號", phone: "07-2866695" },
  { name: "文雄醫院", address: "高雄市三民區察哈爾二街132號", phone: "07-3165275" },
  { name: "長清診所", address: "高雄市三民區九如一路135號", phone: "07-3851441" },
  { name: "德恩診所", address: "高雄市三民區民族一路390號", phone: "07-3958282" },
  { name: "茂田診所", address: "高雄市三民區民誠一路236號", phone: "07-3103127" },
  { name: "建安診所", address: "高雄市三民區大順二路649號", phone: "07-3860999" },
  { name: "高雄新高鳳醫院", address: "高雄市三民區莊敬路288號", phone: "07-3872624" },
  { name: "高悅診所", address: "高雄市三民區九如一路162號1樓", phone: "07-3820123" },
  { name: "義大大昌醫院", address: "高雄市三民區大昌一路305號6樓", phone: "07-5599123#7419" },
  { name: "僾彼高榮育仁診所", address: "高雄市三民區九如一路896號1樓", phone: "07-3891080" },
  { name: "右昌聯合醫院", address: "高雄市楠梓區軍校路930號", phone: "07-3630015" },
  { name: "健仁醫院", address: "高雄市楠梓區楠陽路136號", phone: "07-3529393" },
  { name: "佑強診所", address: "高雄市楠梓區德民路908號", phone: "07-3683687" },
  { name: "楠華診所", address: "高雄市楠梓區興楠路295號", phone: "07-5715101" },
  { name: "興義診所", address: "高雄市楠梓區興楠路342號", phone: "07-3539993" },
  { name: "安馨楠梓內科診所", address: "高雄市楠梓區楠都東街209號", phone: "07-3529393" },
  { name: "佳澤診所", address: "高雄市楠梓區大學東路207號1樓", phone: "07-3601262" },
  { name: "高雄市立小港醫院", address: "高雄市小港區山明路482號", phone: "07-8036783" },
  { name: "佳生診所", address: "高雄市小港區康莊路136號", phone: "07-8053465" },
  { name: "明港診所", address: "高雄市小港區華山路137號", phone: "07-8055116" },
  { name: "高雄榮民總醫院", address: "高雄市左營區大中一路386號", phone: "07-3422121#2051" },
  { name: "國軍左營醫院", address: "高雄市左營區軍校路553號", phone: "07-5810179" },
  { name: "龍華診所", address: "高雄市左營區自由二路67號", phone: "07-5563868" },
  { name: "田源診所", address: "高雄市左營區政德路230號", phone: "07-3432641" },
  { name: "東陽診所", address: "高雄市左營區華夏路626號", phone: "07-3457778" },
  { name: "宗禾診所", address: "高雄市左營區博愛4路80號1樓", phone: "07-3500980" },
  { name: "聖博診所", address: "高雄市左營區博愛三路201號3樓", phone: "07-3496666" },
  { name: "劉內兒科診所", address: "高雄市仁武區灣內村仁心路6號", phone: "07-3710778" },
  { name: "尚清診所", address: "高雄市仁武區中華路245號", phone: "07-3758955" },
  { name: "國軍岡山醫院", address: "高雄市岡山區大義二路1號", phone: "07-6250919" },
  { name: "高雄市立岡山醫院", address: "高雄市岡山區壽天路12號", phone: "07-6243376" },
  { name: "蔣榮福診所", address: "高雄市岡山區中山南路452號", phone: "07-6228719" },
  { name: "仁康診所", address: "高雄市岡山區介壽路74號", phone: "07-6262988" },
  { name: "岡山內科診所", address: "高雄市岡山區岡山路146.148號", phone: "07-6213371" },
  { name: "安泰診所", address: "高雄市岡山區岡山路434號4 F 5B", phone: "07-6233500" },
  { name: "惠川醫院", address: "高雄市岡山區岡山路92號", phone: "07-6229292" },
  { name: "永順診所", address: "高雄市岡山區岡山路72號1樓", phone: "07-6243376" },
  { name: "路竹診所", address: "高雄市路竹區國昌路136號", phone: "07-6975903" },
  { name: "義大醫療財團法人義大醫院", address: "高雄市燕巢區角宿村義大路1號", phone: "07-6150011" },
  { name: "義大癌治療醫院", address: "高雄市燕巢區角宿里義大路21號", phone: "07-615-0011#6550" },
  { name: "湖康診所", address: "高雄市湖內區中正路一段397號", phone: "07-6997126" },
  { name: "高雄市立鳳山醫院", address: "高雄市鳳山區經武路42號", phone: "07-7418151" },
  { name: "嘉美診所", address: "高雄市鳳山區中山路79號", phone: "07-7413668" },
  { name: "長新診所", address: "高雄市鳳山區建國路二段59-2號", phone: "07-7192233" },
  { name: "新華田內科診所", address: "高雄市鳳山區曹公路16-4號", phone: "07-7999128" },
  { name: "佳醫診所", address: "高雄市鳳山區五甲二路357號", phone: "07-7669191" },
  { name: "惠德醫院", address: "高雄市鳳山區福祥街81號", phone: "07-8126000#1202" },
  { name: "高晟診所", address: "高雄市鳳山區鳳東路368號", phone: "07-7823399" },
  { name: "揚銘診所", address: "高雄市鳳山區自由路111號1樓", phone: "07-7905605" },
  { name: "鴻仁健康診所", address: "高雄市鳳山區青年路2段148號1樓", phone: "07-7408333" },
  { name: "健聖診所", address: "高雄市大寮區中正路2號", phone: "07-7861303" },
  { name: "幸安診所", address: "高雄市大寮區鳳林三路526號", phone: "07-7812855" },
  { name: "建佑醫院", address: "高雄市林園區東林西路360號", phone: "07-6418677" },
  { name: "高雄長庚醫院", address: "高雄市鳥松區大埤路123號", phone: "07-7317123" },
  { name: "廣聖醫院", address: "高雄市旗山區中華路618號", phone: "07-6623125" },
  { name: "芳民診所", address: "高雄市旗山區大德里德昌路16號", phone: "07-6622355" },
  { name: "鴻源診所", address: "高雄市旗山區延平一路200號", phone: "07-6628788" },
  { name: "溪洲醫院", address: "高雄市旗山區延平一路412號7F", phone: "07-6622096" },
  { name: "衛生福利部旗山醫院", address: "高雄市旗山區大德里中學路60號", phone: "07-6613811#1535" },
  { name: "永萣診所", address: "高雄市茄萣區白砂路251號", phone: "07-6904979" },
  { name: "壬禾診所", address: "高雄市鼓山區美術南三路82號", phone: "07-5557300" },
  { name: "裕禾診所", address: "高雄市前鎮區南天街125號", phone: "07-3313811" },
  { name: "為好診所", address: "高雄市鼓山區九如四路957號", phone: "07-5215489#112" }
];

// 10. 合作醫院
export const ALLIANCE_HOSPITALS: AllianceHospital[] = [
  {
    name: "高雄市政府衛生局",
    address: "高雄市苓雅區凱旋二路132-1號",
    phone: "07-7134000",
    url: "https://health.kcg.gov.tw/"
  },
  {
    name: "高雄小港衛生局",
    address: "高雄市小港區小港路158號",
    phone: "07-8216939",
    url: "https://sig.kcg.gov.tw/"
  },
  {
    name: "高雄市立小港醫院",
    address: "高雄市小港區山明路482號",
    phone: "07-8036783",
    url: "https://www.kmhk.org.tw/"
  },
  {
    name: "高雄榮民總醫院",
    address: "高雄市左營區大中一路386號",
    phone: "07-3422121",
    url: "https://www.vghks.gov.tw/"
  },
  {
    name: "高雄醫學大學附設醫院",
    address: "高雄市三民區自由一路100號",
    phone: "07-3121101",
    url: "https://www.kmuh.org.tw/"
  },
  {
    name: "高雄長庚紀念醫院",
    address: "高雄市鳥松區大埤路123號",
    phone: "07-7317123",
    url: "https://www.cgmh.org.tw/tw/Systems/AreaInfo/10"
  },
  {
    name: "義大醫療財團法人",
    address: "高雄市燕巢區角宿村義大路1號",
    phone: "07-6150011",
    url: "https://www.edah.org.tw/"
  },
  {
    name: "阮綜合醫院",
    address: "高雄市苓雅區成功一路62號",
    phone: "07-3342765",
    url: "http://www.yuanhosp.com.tw/"
  },
  {
    name: "健仁醫院",
    address: "高雄市楠梓區楠陽路136號",
    phone: "07-3529393",
    url: "https://www.jiannren.org.tw/"
  },
  {
    name: "國軍高雄總醫院",
    address: "高雄市苓雅區中正一路2號",
    phone: "07-7496592",
    url: "https://802.mnd.gov.tw/"
  }
];